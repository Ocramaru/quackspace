...
def _compute_dirty(
    ...
    current_rels: set[str] = set()  # consumed by the vanished-file check below
    for e in space.entries:
        current_rels.add(e.rel)
...
def _fast_noop(
    ...
    *,
    ...
    opaque_dirs: "frozenset | None" = None,
    progress: Callable[[int, int, str], None] | None = None,
) -> dict | None:
    ...
    files, folders_now, newest_marker_ns = scan_signature(
        root, dataset_policy=dataset_policy, opaque_dirs=opaque_dirs, progress=progress
    )
...
def reindex(
    ...
    * **full** — a file was added/removed or its name/body/description changed:
    ...
    dataset_policy = DatasetPolicy(
        total=config.index.dataset_threshold,
        per_ext=config.index.dataset_ext_threshold,
        extensions=DEFAULT_DATASET_EXTENSIONS | frozenset(config.index.dataset_extensions),
    )
    opaque_dirs = DEFAULT_OPAQUE_DIRS | frozenset(config.index.opaque_dirs)
    catalog_path = root / ".quack" / catalog.DB_NAME
    catalog_mode_changed = not catalog.store_body_matches(catalog_path, store_body)
    noop = _fast_noop(
        root, store_body=store_body, dataset_policy=dataset_policy,
        opaque_dirs=opaque_dirs, progress=progress,
    )
    if noop is not None:
    ...
    folder_infos = folders.resolve_folders(space, config.index.tag_rollup_limit)
